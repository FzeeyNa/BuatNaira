...
let renderer = new THREE.WebGLRenderer({
    canvas: canvasEl,
    alpha: true,
    preserveDrawingBuffer: true // Required for saving screenshot properly
});
...
window.addEventListener("click", (e) => {
    if(e.target.id === 'screenshot') return; // Don't trigger drawing toggle when clicking save button
    pointer.x = e.pageX / window.innerWidth;
    pointer.y = e.pageY / window.innerHeight;
...
screenshotBtn.addEventListener('click', () => {
    ...
    canvasEl.toBlob((blob) => {
        saveBlob(blob, "flowers-for-naira.png");
    });
